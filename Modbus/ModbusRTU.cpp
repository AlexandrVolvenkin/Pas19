...
{
    m_pxSerialPort = new CSerialPort();
    SetFsmState(IDDLE);
}
...
CModbusRtu::~CModbusRtu()
{
    delete m_pxSerialPort;
}

//-----------------------------------------------------------------------------------------------------
void CModbusRtu::SerialPortInit(const char* pccPortName,
                                uint32_t uiBaudRate,
                                uint8_t uiDataBits,
                                char cParity,
                                uint8_t uiStopBit)
{
    m_pxSerialPort -> Init();
    m_pxSerialPort -> SetPortName(pccPortName);
    m_pxSerialPort -> SetBaudRate(uiBaudRate);
    m_pxSerialPort -> SetDataBits(uiDataBits);
    m_pxSerialPort -> SetParity(cParity);
    m_pxSerialPort -> SetStopBit(uiStopBit);
...
{
    return m_pxSerialPort -> Write(puiMessage, uiLength);
}

//-----------------------------------------------------------------------------------------------------
int16_t CModbusRtu::Receive(uint8_t *puiDestination, uint16_t uiLength)
{
    return m_pxSerialPort -> Read(puiDestination, uiLength);
}
